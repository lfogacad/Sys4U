...
const navButtons = allNavButtons.filter((btn) => {
  ...
  const ModuloRecepcao = ({ userProfile }) => {
    ...
    // Estados do Cadastro
    const [view, setView] = useState('search'); // Controla se mostra a busca ou o formulário
    ...
    const handleOpenRegister = () => {
      // Preenche o CPF já digitado na busca para economizar tempo
    ...
    // ==========================================
    // TELA 1: BUSCA DE PACIENTES (Original com ajuste no botão)
    // ==========================================
    return (
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
              <input
                ...
                className="flex-1 p-4 border border-slate-200 rounded-xl bg-slate-50 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 text-lg font-mono tracking-widest"
                value={cpf}
          ...
          {status === 'found' && patient && (
            <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1">Data Nasc: <b>{formatarDataBR(patient.nascimento)}</b> | CPF: <b>{patient.cpf}</b></p>
              </div>
              <button 
          ...
          {status === 'not_found' && (
            <div className="bg-white p-10 rounded-2xl shadow-sm border border-dashed border-slate-300 text-center">
              ...
              {/* O BOTÃO QUE ACIONA A TELA DE CADASTRO */}
...
// B.2. CATRACA DE SEGURANÇA GLOBAL: Primeiro Acesso
  // Se o médico fez o login, mas a ficha diz que é a primeira vez, bloqueia TUDO!
  if (userProfile && userProfile.isFirstLogin === true) {
    return (
...
}; // Este fecha a função const AppRouter = () => {
