...
import { UserPlus, X, AlertTriangle, FileText } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[95vh] overflow-y-auto flex flex-col shadow-2xl">
        <div className="bg-green-600 p-4 text-white flex justify-between items-center sticky top-0 z-10 shadow">
          <h3 className="font-bold flex items-center gap-2 text-lg">
            <UserPlus size={20} /> Admissão de Enfermagem (Leito {activeTab + 1})
          </h3>
          <button onClick={() => setShowNursingModal(false)} className="hover:bg-green-700 p-1 rounded transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6 text-sm">
          ...
          <div className="p-4 border rounded-xl bg-orange-50/20 shadow-sm">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                ...
                <select className="w-full p-2 border rounded" value={nursingData.dor || ""} onChange={(e) => setNursingData({ ...nursingData, dor: e.target.value })}>
              ...
              <div className="flex items-center">
                <label className="flex items-center gap-2 font-bold text-slate-700">
                  <input type="checkbox" className="w-4 h-4" checked={nursingData.hemodialise || false} onChange={(e) => setNursingData({ ...nursingData, hemodialise: e.target.checked })} /> Hemodiálise
                </label>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Precauções</label>
                <select className="w-full p-2 border rounded" value={nursingData.precaucao || ""} onChange={(e) => setNursingData({ ...nursingData, precaucao: e.target.value })}>
                  <option value="">Selecione...</option>
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input className="w-full p-2 border rounded" placeholder="Local" value={nursingData.avpLocal || ""} onChange={(e) => setNursingData({ ...nursingData, avpLocal: e.target.value })} />
                  <input type="date" className="w-40 p-2 border rounded" value={nursingData.avpData || ""} onChange={(e) => setNursingData({ ...nursingData, avpData: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 mb-1 block">CVC/PICC (Local/Data)</label>
                <div className="flex gap-2">
                  <input className="w-full p-2 border rounded" placeholder="Local" value={nursingData.cvcLocal || ""} onChange={(e) => setNursingData({ ...nursingData, cvcLocal: e.target.value })} />
                  <input type="date" className="w-40 p-2 border rounded" value={nursingData.cvcData || ""} onChange={(e) => setNursingData({ ...nursingData, cvcData: e.target.value })} />
                </div>
              </div>
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                <label className="flex items-center gap-2 text-xs font-bold text-gray-500 mb-1">
                  <input type="checkbox" checked={nursingData.svd || false} onChange={(e) => setNursingData({ ...nursingData, svd: e.target.checked })} /> SVD (Data)
                </label>
                <input type="date" className={`w-full p-2 border rounded ${!nursingData.svd ? "bg-gray-100 opacity-50" : ""}`} value={nursingData.svdData || ""} onChange={(e) => setNursingData({ ...nursingData, svdData: e.target.value })} disabled={!nursingData.svd} />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 mb-1 block">SNE (Fixação cm / Data)</label>
                <div className="flex gap-2">
                  <input className="w-full p-2 border rounded" placeholder="cm" value={nursingData.sneCm || ""} onChange={(e) => setNursingData({ ...nursingData, sneCm: e.target.value })} />
                  <input type="date" className="w-32 p-2 border rounded" value={nursingData.sneData || ""} onChange={(e) => setNursingData({ ...nursingData, sneData: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 mb-1 block">Drenos</label>
                <input className="w-full p-2 border rounded" placeholder="Tipo/Características" value={nursingData.drenoTipo || ""} onChange={(e) => setNursingData({ ...nursingData, drenoTipo: e.target.value })} />
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border rounded-xl bg-orange-50/20 shadow-sm">
            ...
            <textarea placeholder="Lesões por pressão (Local / Estágio)..." className="w-full p-2 border rounded mb-3 h-16 outline-none focus:ring-2 focus:ring-orange-200" value={nursingData.lesaoLocal || ""} onChange={(e) => setNursingData({ ...nursingData, lesaoLocal: e.target.value })} />
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-xs font-bold text-gray-500 mb-1 block">Tipo de Curativo</label>
                <input className="w-full p-2 border rounded" placeholder="Descritivo do curativo" value={nursingData.curativoTipo || ""} onChange={(e) => setNursingData({ ...nursingData, curativoTipo: e.target.value })} />
              </div>
              <div className="w-40">
                <label className="text-xs font-bold text-gray-500 mb-1 block">Data Curativo</label>
                <input type="date" className="w-full p-2 border rounded" value={nursingData.curativoData || ""} onChange={(e) => setNursingData({ ...nursingData, curativoData: e.target.value })} />
              </div>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {Object.entries(BRADEN_OPTIONS).map(([key, options]) => (
                <div key={key}>
                  ...
                  <select className="w-full p-2.5 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-orange-300" value={nursingData[`braden_${key}`] || ""} onChange={(e) => setNursingData({ ...nursingData, [`braden_${key}`]: e.target.value })}>
                    <option value="">Selecione...</option>
                    {options.map((opt) => (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {Object.entries(MORSE_OPTIONS).map(([key, options]) => (
                <div key={key}>
                  ...
                  <select className="w-full p-2.5 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-blue-300" value={nursingData[`morse_${key}`] || ""} onChange={(e) => setNursingData({ ...nursingData, [`morse_${key}`]: e.target.value })}>
                    <option value="">Selecione...</option>
                    {options.map((opt) => (
                      <option key={opt.value} value={opt.value}>{opt.label}</option>
                    ))}
