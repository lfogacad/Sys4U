  ...
  try {
    ...
    leitosSnapshot.forEach((doc) => {
      ...
      if (p.nome && p.status !== "Livre") {
        ...
        } // FIM DO BLOCO PROTEGIDO (isFechamentoDiario)

        // Adiciona a promessa de atualização do leito (que sempre atualiza o O2, e se for 23h, reseta a enfermagem)
        promessasAuditoria.push(doc.ref.update(updatePayload));
    ...
    if (isFechamentoDiario) {
      ...
      console.log(`✅ Fechamento concluído com sucesso. Casos capturados nesta madrugada: PAV=${novosCasosPAV}, IPCS-C=${novosCasosIPCSC}, ITU-AC=${novosCasosITU}. Todos os leitos resetados e censo salvo.`);
    } else {
